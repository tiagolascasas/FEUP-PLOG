...
checkSpecialMove :- waiterPos(Table, _),
		      specMovePos(SpecMove, Table, Amount, TargetP), currentPiece(Player), getPieceCountOnTable(Player, Table, N),
		      ((N >= Amount) -> write('Special Move Unlocked: '), nl, G =.. [SpecMove, Amount, TargetP], G; !).
